...
if (!function_exists('orderan')) {
    function orderan($id, $sts)
    {
        ...
        $CI->db->select('pesanan.*, detail_order.harga, detail_order.qty');
        $CI->db->from('pesanan');
        $CI->db->join('detail_order', 'detail_order.order_id = pesanan.idorder');
        $CI->db->where('pesanan.user_id', $id);
        $CI->db->where('pesanan.status', $sts);

...
class User_m extends CI_Model
{
	...
	{
		$sql = "SELECT users.*, pesanan.*, pembayaran.status AS verify, detail_order.harga, detail_order.qty 
            FROM users 
            JOIN pesanan ON pembayaran.order_id = pesanan.idorder
            JOIN detail_order ON pesanan.idorder = detail_order.order_id";
		$this->db->join('users', 'users.idusers = pesanan.user_id', 'left');
		return $this->db->get('pesanan')->result();
	}
	public function allPayment()
	{
		//$this->db->join('detail_order', 'detail_order.iddetail_order = pembayaran.order_id', 'left');
		// $this->db->join('pesanan', 'pesanan.idorder = pembayaran.order_id', 'left');
		//$sql = "SELECT * FROM detail_order JOIN pembayaran ON detail_order.order_id = pembayaran.order_id";
		...
		$sql = "SELECT pembayaran.*, pesanan.*, pembayaran.status AS verify, detail_order.harga, detail_order.qty 
            FROM pembayaran 
            JOIN pesanan ON pembayaran.order_id = pesanan.idorder
            JOIN detail_order ON pesanan.idorder = detail_order.order_id";
